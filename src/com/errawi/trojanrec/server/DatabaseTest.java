...
public class DatabaseTest {
	...
	public static void main(String[] args) {
		...
		/*
		 *
		 * Testing DatabaseHandler authenticateUser() method Returns boolean 'true' if
		 * net_id+password combination exists
		 *
		 */
		...
		/*
		 *
		 * Testing DatabaseHandler retrieveUser() method Returns populated User object
		 *
		 */
		User user;
		...
		System.out.println(
				user.getName() + ", " + user.getStudentID() + ", " + user.getUserPhoto() + ", " + user.getNetID());
		...
		System.out.println(user.getName() + ", " + user.getStudentID() + ", " + user.getUserPhoto() + ", "
				+ user.getNetID() + "[empty test 1]");
		user = db.retrieveUser("");
		System.out.println(user.getName() + ", " + user.getStudentID() + ", " + user.getUserPhoto() + ", "
				+ user.getNetID() + "[empty test 2]");
		user = db.retrieveUser(null);
		System.out.println(user.getName() + ", " + user.getStudentID() + ", " + user.getUserPhoto() + ", "
				+ user.getNetID() + "[empty test 3 - null]");
		...
		/*
		 *
		 * Testing DatabaseHandler getCenterTimeslots() method Returns populated all
		 * center timeslots for specified gym
		 *
		 */
		String result;
		...
		/*
		 * Testing DatabaseHandler addToWaitlist() method Return value void, adds user
		 * to the waitlist
		 *
		 */
		...
		/*
		 * Testing DatabaseHandler getWaitlist() method Returns ArrayList of Users in
		 * the waitlist for given time/center
		 *
		 */
		...
		/*
		 * Testing DatabaseHandler clearWaitlist() method removes all reservations
		 * corresponding with a center+datetime
		 */
		...
		/*
		 * 
		 * Testing DatabaseHandler makeBooking() method clear bookings first (for
		 * testing purposes)
		 *
		 */
		...
		db.makeBooking(1, "2022-03-25 06:00:00", db.retrieveUser("erin")); // won't make the second booking - should
																			// return an error
		 ...
		 * NOT DONE TESTING
		 *
		 * Testing DatabaseHandler isCapMax() method Returns true if current reservation
		 * capacity and max capacity are the same
		 *
		 */
		boolean cap;
		...
		db.removeBooking(1, "2022-03-25 06:00:00", db.retrieveUser("avonlea")); // should return an error, won't delete
																				// twice
		...
		System.out.println("\n**** TESTING DONE ****");
		/*
		 * close pool after database calls are complete do not make any database calls
		 * using DatabaseHandler db after it is closed
		 */
		db.datasource.close();
	}
}
