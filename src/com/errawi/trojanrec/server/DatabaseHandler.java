...
public class DatabaseHandler {
	...
	public synchronized boolean authenticateUser(String net_id, String password) {
		...
		try {
			...
			PreparedStatement pst = conn.prepareStatement("SELECT * FROM trojanrec.userauth " + "WHERE net_id = '"
	...
	public synchronized User retrieveUser(String net_id) {
		...
		try {
			...
			PreparedStatement pst = conn.prepareStatement("SELECT name, student_id, image_path, net_id "
	 ...
	 * @return String printout of timeslots (can format in a more desirable way -
	 *         ArrayList?)
	 *
	 */
	public synchronized String getCenterTimeslots(int center_id) {
		...
		try {
			...
			PreparedStatement pst = conn.prepareStatement("SELECT reservation_time " + "FROM trojanrec.timeslot "
					+ "WHERE center_id = '" + center_id + "' ORDER BY reservation_time");
			ResultSet rs = pst.executeQuery();
	...
	public synchronized boolean isCapMax(int center_id, String timedate) { // what inputs do we want? this okay?
		...
		try {
			...
			PreparedStatement pst = conn.prepareStatement("SELECT cap_max, cap_curr " + "FROM trojanrec.timeslot "
	...
	public synchronized void addToWaitlist(int center_id, String timedate, User user) {
		try {
			...
			PreparedStatement pst = conn.prepareStatement("SELECT timeslot_id " + "FROM trojanrec.timeslot "
					+ "WHERE center_id = '" + center_id + "' AND reservation_time = '" + timedate + "'");
			...
			if (rs.next()) {
				...
				pst_k = conn.prepareStatement(
						"SELECT user_id " + "FROM trojanrec.userinfo " + "WHERE name = '" + user.getName() + "'");
				rs_k = pst_k.executeQuery();
				if (rs_k.next()) {
					String sql = "INSERT INTO trojanrec.waitlist(timeslot_id, user_id) " + "VALUES ('" + timeslot_id
							+ "', '" + rs_k.getInt("user_id") + "')";
		...
		try {
			...
			if (rs.next()) {
				...
				if (rs_k.next()) {
					...
					pst_j = conn.prepareStatement("SELECT EXISTS(SELECT * " + "FROM trojanrec.booking "
					...
					if (booking_made == 0) {
						String sql = "INSERT INTO trojanrec.booking(timeslot_id, user_id) " + "VALUES ('" + timeslot_id
								+ "', '" + rs_k.getInt("user_id") + "')";
						stmt = conn.createStatement();
						stmt.executeUpdate(sql);
						...
						log.info(
								"A user tried to make a duplicate booking - this is not allowed! No futher action is necessary :-)");
					}
				}
			}
		} catch (SQLException e) {
		...
		try {
			...
			if (rs.next()) {
				...
				if (rs_k.next()) {
					...
					// cancel a booking you haven't made, but keeping this in here for now just in
					// case
					pst_j = conn.prepareStatement("SELECT EXISTS(SELECT * " + "FROM trojanrec.booking "
							+ "WHERE timeslot_id = '" + timeslot_id + "' AND user_id = '" + userID + "')");
					rs_j = pst_j.executeQuery();
					...
					if (booking_made != 0) {
						String sql = "DELETE FROM trojanrec.booking " + "WHERE timeslot_id = '" + timeslot_id
								+ "' AND user_id = '" + userID + "'";
						stmt = conn.createStatement();
						stmt.executeUpdate(sql);
						...
						log.info(
								"A user tried to remove a booking, but they didn't have a booking - this is not allowed! No futher action is necessary :-)");
					}
				}
			}
		} catch (SQLException e) {
			...
			if (rs.next()) {
				...
				while (rs_k.next()) {
					...
					PreparedStatement pst_j = conn.prepareStatement("SELECT reservation_time "
							+ "FROM trojanrec.timeslot " + "WHERE timeslot_id = '" + rs_k.getInt("timeslot_id") + "'");
		...
		try {
			...
			if (rs.next()) {
				...
				while (rs_k.next()) {
					...
					while (rs_j.next()) {
						int compare = date_now.compareTo(rs_j.getString("reservation_time"));
						// if current datetime is greater than or equal to a booking time, booking is in
						// past
						if (compare >= 0) {
							bookings.add(rs_j.getString("reservation_time"));
						}
	...
	public synchronized ArrayList<User> getWaitlist(int center_id, String timedate) {
		...
		try {
			...
			if (rs.next()) {
				pst_j = conn.prepareStatement("SELECT user_id " + "FROM trojanrec.waitlist " + "WHERE timeslot_id = '"
	 ...
	 *
	 * @param center_id The rec center's id: 1 is Lyon, 2 is Cromwell, 3 is Village
	 * @param timedate  Time and date of the gym timeslot clears the entries in the
	 *                  waitlist for a specific center
	 *
	 */
	public synchronized void clearWaitlist(int center_id, String timedate) {
		try {
			...
			if (rs.next()) {
				String sql = "DELETE FROM trojanrec.waitlist " + "WHERE timeslot_id = '" + rs.getString("timeslot_id")
						+ "'";
	...
	/**
	 *
	 * clears the entire bookings table - for testing suite purposes also makes all
	 * curr capacities be 0 in timeslot table
	 *
	 */
